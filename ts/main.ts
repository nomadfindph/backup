...
class NomadfindPH {
    ...
    public signUpTenant(name: string, username: string, email: string, password: string) {
        let tenant = new Tenant(name, username, email, password);
        this.tenants.push(tenant);
        console.log(`Successfully registered tenant ${tenant.name}`);
    }

    public signUpLandlord(name: string, username: string, email: string, password: string, proof: Image) {
        let landlord = new Landlord(name, username, email, password, proof);
        this.landlords.push(landlord);
        console.log(`Successfully registered landlord ${landlord.name}`);
    }
...
// PRINTING TENANDT AND LANDLORD NAMES
...
app.tenants[0].logIn("", "")
console.log("\nWelcome to Log in Page!");
app.tenants[0].logIn("seannamo.01", "alliznotwell123")
